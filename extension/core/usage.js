 ...
 */
export function partition_rules(rules) {
  const enforced = [];
  const allowance = [];
...
export function bump_usage(counts, rule_id, value, n = 1) {
 ...
 */
export function bump_bounded(counts, key, n = 1, limit = 256) {
  if (
    !Number.isInteger(n) ||
    n <= 0 ||
 ...
 */
export function usage_entries(counts, limit = 128) {
  const entries = [];
...
export function usage_snapshot(counts) {
  const out = {};
  for (const [key, count] of counts) {
    const decoded = decode_usage_key(key);
...
export function retire_usage(counts, entries) {
 ...
 */
export function merge_labels(counts, labels, blocked_prefixes = []) {
...
export function prune_usage(counts, rules) {
...
export function merge_usage(counts, entries) {
  for (const entry of entries) {
 ...
 */
export function restore_usage(counts, stored) {
